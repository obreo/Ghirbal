...
Allows users to add custom domains to the blocklist through local files, manual input, or remote URLs integrated with EasyList, Adguard, and uBlock Origin filters. Look at `client/lib/Blocklist/blocklist.ts` and `client/lib/Blocklist/endpoints.ts` for more details.
