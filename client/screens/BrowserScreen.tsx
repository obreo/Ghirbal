...
const getAppSchemeForUrl = (url: string): { scheme: string; name: string; deepLink: string } | null => {
  try {
    ...
    if (host.includes('github.com')) {
      // GitHub app uses github:// scheme with the full path
      return { scheme: 'github://', name: 'GitHub', deepLink: `github://${host}${pathname}${search}` };
    }
...
export default function BrowserScreen() {
  ...
  const [errorDetails, setErrorDetails] = useState({ domain: '', code: '', description: '' });
  ...
  const facebookDeepLinkPreventionJS = `
    (function() {
      // Prevent Facebook from initiating deep link redirects
      if (location.hostname.includes('facebook.com') || location.hostname.includes('fb.com')) {
        // Override window.location assignment to prevent deep link redirects
        const originalLocation = window.location;
        const locationProxy = new Proxy({}, {
          get: function(target, prop) {
            if (prop === 'href' || prop === 'replace' || prop === 'assign') {
              return function(url) {
                if (typeof url === 'string' && (url.startsWith('fb://') || url.startsWith('intent://'))) {
                  console.log('Blocked Facebook deep link attempt:', url);
                  // Don't allow the deep link to proceed
                  return;
                }
                ...
                if (prop === 'href') {
                  ...
                  return originalLocation[prop].apply(originalLocation, arguments);
                }
              };
            }
            return originalLocation[prop];
          ...
          set: function(target, prop, value) {
            if ((prop === 'href' || prop === 'replace' || prop === 'assign') &&
                typeof value === 'string' &&
                (value.startsWith('fb://') || value.startsWith('intent://'))) {
              console.log('Blocked Facebook deep link assignment:', value);
              return true; // Silently ignore the assignment
            }
            originalLocation[prop] = value;
            return true;
          }
        });
        ...
        // Intercept all anchor clicks to prevent deep link navigation
        document.addEventListener('click', function(e) {
          ...
          if (target && target.href) {
            if (target.href.startsWith('fb://') || target.href.startsWith('intent://')) {
              e.preventDefault();
              e.stopPropagation();
              console.log('Blocked Facebook deep link anchor:', target.href);
              return false;
            }
          }
        }, true);

        // Override setTimeout/setInterval to prevent delayed deep link attempts
        const originalSetTimeout = window.setTimeout;
        window.setTimeout = function(callback, delay) {
          // Check if the callback tries to initiate a deep link
          if (typeof callback === 'string' &&
              (callback.includes('fb://') || callback.includes('intent://'))) {
            console.log('Blocked deep link in setTimeout');
            return originalSetTimeout(function() {}, delay); // Schedule empty function instead
          }
        ...
        window.setInterval = function(callback, interval) {
          if (typeof callback === 'string' &&
              (callback.includes('fb://') || callback.includes('intent://'))) {
            console.log('Blocked deep link in setInterval');
            return originalSetInterval(function() {}, interval); // Schedule empty function instead
          }
          return originalSetInterval.apply(this, arguments);
        };

        // Override window.open to prevent deep link opening
        const originalOpen = window.open;
        window.open = function(url, target, features) {
          if (typeof url === 'string' &&
              (url.startsWith('fb://') || url.startsWith('intent://'))) {
            console.log('Blocked Facebook deep link in window.open:', url);
            return null; // Return null instead of opening
          }
          return originalOpen.call(this, url, target, features);
        };

        // Override document.location to prevent deep link navigation
        ...
        Object.defineProperty(document, 'location', {
          ...
          get: function() {
            if (originalDocumentLocation &&
                (originalDocumentLocation.href.startsWith('fb://') ||
                 originalDocumentLocation.href.startsWith('intent://'))) {
              console.log('Blocked Facebook deep link in document.location:', originalDocumentLocation.href);
              return originalDocumentLocation; // Return original but don't allow deep link
            }
          ...
          set: function(value) {
            if (typeof value === 'string' &&
                (value.startsWith('fb://') || value.startsWith('intent://'))) {
              console.log('Blocked Facebook deep link in document.location assignment:', value);
              return; // Don't allow the assignment
            }
            originalDocumentLocation.href = value;
          }
        });

        // Additional protection: Monitor for attempts to change the URL programmatically
        const observer = new MutationObserver(function(mutations) {
          mutations.forEach(function(mutation) {
            if (mutation.type === 'attributes' && mutation.attributeName === 'href') {
              const element = mutation.target;
              if (element.tagName === 'A' &&
                  (element.href.startsWith('fb://') || element.href.startsWith('intent://'))) {
                console.log('Blocked mutated Facebook deep link:', element.href);
                element.href = 'javascript:void(0);'; // Neutralize the link
              }
            }
          });
        });
  ...
  const handleUrlSubmit = useCallback(() => {
    ...
    if (result.blocked) {
      ...
      setForceNavCounter(c => c + 1); // Force WebView to navigate to the new URL or reflect blocked state
      return;
    }
    ...
    setForceNavCounter(c => c + 1); // Force WebView to navigate to the new URL
  }, [urlInputValue, activeTabId, updateTab, setForceNavCounter]);
  ...
  const handleNavigationStateChange = useCallback((navState: any) => {
    ...
    if (!navState.url.startsWith('fb://') &&
        ...
        !navState.url.startsWith('javascript:')) {
      lastValidUrl.current = navState.url;
    }
    ...
    const result = processUrl(navState.url, 'navigation');
      ...
      setTimeout(() => {
        if (webViewRef.current) {
          ...
          webViewRef.current.injectJavaScript(`
            ...
            // More aggressive deep link prevention script
            (function() {
              // Prevent Facebook from initiating deep links
              if (location.hostname.includes('facebook.com') || location.hostname.includes('fb.com')) {
                // Override window.location assignment to prevent deep link redirects
                ...
                Object.defineProperty(window, 'location', {
                  ...
                  set: function(value) {
                    ...
                    if (strValue.startsWith('fb://') || strValue.startsWith('intent://')) {
                      console.log('Blocked deep link attempt:', strValue);
                      // Don't allow the deep link to proceed
                      return;
                    }
                ...
                document.addEventListener('click', function(e) {
                  ...
                  if (target && target.href) {
                    if (target.href.startsWith('fb://') || target.href.startsWith('intent://')) {
                      e.preventDefault();
                      e.stopPropagation();
                      console.log('Blocked deep link anchor:', target.href);
                      return false;
                    }
                  }
                }, true);
                ...
                window.open = function(url, name, features) {
                  if (typeof url === 'string' &&
                      (url.startsWith('fb://') || url.startsWith('intent://'))) {
                    console.log('Blocked deep link in window.open:', url);
                    return null;
                  }
                ...
                var originalHref = Object.getOwnPropertyDescriptor(Document.prototype, 'location').set;
                Object.defineProperty(document, 'location', {
                  configurable: true,
                  get: function() {
                  ...
                  set: function(value) {
                    var strValue = String(value);
                    if (strValue.startsWith('fb://') || strValue.startsWith('intent://')) {
                      console.log('Blocked deep link in document.location:', strValue);
                      return;
                    }
                    if (originalHref) {
                      originalHref.call(document, value);
                    }
                  }
                });
              }

              // Similar prevention for other social media sites
                ...
                document.addEventListener('click', function(e) {
                  ...
                  if (target && target.href) {
                    if (target.href.startsWith('instagram://')) {
                      e.preventDefault();
                      e.stopPropagation();
                      console.log('Blocked Instagram deep link:', target.href);
                      return false;
                    }
                  }
                }, true);
              ...
              if (location.hostname.includes('twitter.com') || location.hostname.includes('x.com')) {
                document.addEventListener('click', function(e) {
                  ...
                  if (target && target.href) {
                    if (target.href.startsWith('twitter://')) {
                      e.preventDefault();
                      e.stopPropagation();
                      console.log('Blocked Twitter deep link:', target.href);
                      return false;
                    }
                  }
                }, true);
  ...
  const handleLoadEnd = useCallback((_event?: any) => {
    ...
    // Reset the flag when loading ends to allow future blocking operations
    justBlockedUrl.current = false;

    // Clear error state when page loads successfully
    if (hasError) {
      setHasError(false);
      setErrorMessage('');
      setErrorDetails({ domain: '', code: '', description: '' });
    }
  ...
  // State to track recent deep link attempts to prevent loops
  const recentDeepLinkAttempts = useRef<Map<string, number>>(new Map());
  ...
  // State to track if we just blocked a URL to prevent duplicate goBack calls
  const justBlockedUrl = useRef<boolean>(false);

  // Function to convert deep links to web URLs as fallback
  const convertDeepLinkToWebUrl = useCallback((url: string): string | null => {
    try {
      // Handle Facebook deep links
      if (url.startsWith('fb://')) {
        // Extract video ID from fb://fullscreen_video/{id}
        const fbVideoMatch = url.match(/fb:\/\/fullscreen_video\/([^?]+)/);
        if (fbVideoMatch) {
          const videoId = fbVideoMatch[1];
          return `https://mbasic.facebook.com/watch/?v=${videoId}`;
        }

        // Handle native post pattern like fb://native_post/UzpfSTEwMDA2MzY1MDE5NDY3Nzo5MzIyOTk5MDg5MDE2NzQ6OTMyMjk5OTA4OTAxNjc0?wtsid=seo_0UlubaioMMUodcXkB
        // Pattern: fb://native_post/UzpfST{userId}:{postId}:{someId}?wtsid=...
        const fbNativePostMatch = url.match(/fb:\/\/native_post\/UzpfST(\d+):(\d+):(\d+)\?wtsid=/);
        if (fbNativePostMatch) {
          const userId = fbNativePostMatch[1];
          const postId = fbNativePostMatch[2];
          return `https://mbasic.facebook.com/${userId}/posts/${postId}`;
        }

        // Handle native post pattern like fb://native_post/UzpfSTEwMDA2MzY1MDE5NDY3Nzo5MzIyOTk5MDg5MDE2NzQ6OTMyMjk5OTA4OTAxNjc0?wtsid=seo_0UlubaioMMUodcXkB (alternative format)
        const fbNativePostMatchAlt = url.match(/fb:\/\/native_post\/UzpfSTEwMDA2MzY1MDE5NDY3Nzo(\d+):(\d+)\?wtsid=/);
        if (fbNativePostMatchAlt) {
          const userId = "100063650194677"; // Extracted from the pattern
          const postId = fbNativePostMatchAlt[1];
          return `https://mbasic.facebook.com/${userId}/posts/${postId}`;
        }

        // Handle native post pattern more generally: fb://native_post/UzpfST{numbersSeparatedByColons}?wtsid=...
        const fbNativePostGeneral = url.match(/fb:\/\/native_post\/UzpfST([^?]+)\?wtsid=/);
        if (fbNativePostGeneral) {
          const idParts = fbNativePostGeneral[1].split(':');
          if (idParts.length >= 2) {
        ...
        // Handle other native post patterns
        const fbNativePostMatch2 = url.match(/fb:\/\/(post|story|photo|video)\/([^?]+)/);
        if (fbNativePostMatch2) {
          const type = fbNativePostMatch2[1];
          const id = fbNativePostMatch2[2];

          switch (type) {
            case 'post':
              // Extract user ID and post ID from the complex ID format
              const ids = id.split(':');
              if (ids.length >= 2) {
                return `https://mbasic.facebook.com/${ids[0]}/posts/${ids[1]}`;
              }
              return `https://mbasic.facebook.com/${id}`;
            case 'story':
              return `https://mbasic.facebook.com/story.php?story_fbid=${id}`;
            case 'photo':
              return `https://mbasic.facebook.com/photo/?fbid=${id}`;
            case 'video':
              return `https://mbasic.facebook.com/watch/?v=${id}`;
            default:
              return `https://mbasic.facebook.com/${id}`;
          }
        }

        // Handle any fb://native_post/... pattern as a general fallback
        if (url.startsWith('fb://native_post/')) {
          // Extract the part after 'fb://native_post/'
          const postPath = url.replace('fb://native_post/', '').split('?')[0];
          // Try to extract user and post IDs from the complex identifier
          const parts = postPath.split(':');
          if (parts.length >= 2) {
            // Format is typically UzpfST{userId}:{postId}:{extraId}
            // Remove the UzpfST prefix if present
            let userId = parts[0];
            let postId = parts[1];

            if (userId.startsWith('UzpfST')) {
              userId = userId.replace('UzpfST', '');
            }
            if (postId.startsWith('UzpfST')) {
              postId = postId.replace('UzpfST', '');
            }
        ...
        // Handle other Facebook deep links
        const fbMatch = url.match(/fb:\/\/(profile|page|photo|video|post|reel)\/([^?]+)/);
        if (fbMatch) {
          const type = fbMatch[1];
          const id = fbMatch[2];
          ...
          switch (type) {
            case 'profile':
              return `https://mbasic.facebook.com/${id}`;
            case 'page':
              return `https://mbasic.facebook.com/pages/${id}`;
            case 'photo':
              return `https://mbasic.facebook.com/photo/?fbid=${id}`;
            case 'video':
              return `https://mbasic.facebook.com/watch/?v=${id}`;
            case 'post':
              return `https://mbasic.facebook.com/${id}`;
            case 'reel':
              return `https://mbasic.facebook.com/reel/${id}`;
            default:
              return `https://mbasic.facebook.com/${id}`;
          }
        }

        // Handle generic fb:// links by extracting the path
        const genericFbMatch = url.match(/fb:\/\/([^?]+)/);
        if (genericFbMatch) {
          const path = genericFbMatch[1];
          return `https://mbasic.facebook.com/${path}`;
        }
      }

      // Handle Instagram deep links
      if (url.startsWith('instagram://')) {
        const igMatch = url.match(/instagram:\/\/(p|stories|user)\/([^?]+)/);
        if (igMatch) {
          ...
          switch (type) {
            ...
            case 'stories':
              return `https://www.instagram.com/stories/${id}/`;
            case 'user':
      ...
      // Handle Twitter/X deep links
      if (url.startsWith('twitter://')) {
        const twitterMatch = url.match(/twitter:\/\/(user|status)\/([^?]+)/);
        if (twitterMatch) {
          const type = twitterMatch[1];
      ...
      if (url.startsWith('whatsapp://')) {
        const waMatch = url.match(/whatsapp:\/\/(send|chat)\/\?phone=(\d+)/);
        if (waMatch) {
          const phone = waMatch[2];
      ...
      console.error('Error converting deep link to web URL:', error);
      return null;
    }
  }, []);

  // Function to map error codes to friendly messages
  const getFriendlyErrorMessage = useCallback((errorCode: number, description: string, url: string): string => {
    // Common Android WebView error codes
    switch (errorCode) {
      case -2:
        return "The URL is not valid. Please check the address and try again.";
      case -3:
        return "The connection timed out. Please check your internet connection and try again.";
      case -4:
        return "The server is temporarily unavailable. Please try again later.";
      case -5:
        return "The server could not be found. Please check your internet connection.";
      case -6:
        return "The connection failed. Please check your internet connection.";
      case -7:
        return "The SSL certificate is invalid. This website may not be secure.";
      case -8:
        return "The connection was lost. Please check your internet connection.";
      case -9:
        return "Access to this website is forbidden.";
      case -10:
        return "The server is not responding. Please try again later.";
      case -11:
        return "The server reset the connection. Please try again.";
      case -12:
      ...
      case -16:
        return "The download failed due to unsupported authentication scheme.";
      case -17:
        return "The download failed due to unsupported proxy authentication.";
      case -18:
        return "The certificate date is invalid.";
      case -19:
        return "The download failed due to access denial.";
      case -20:
        return "The download failed due to failed SSL handshake.";
      case -21:
        return "The download failed due to bad URL.";
      case -22:
        return "The download failed due to file access error.";
      case -23:
        return "The download failed due to too many redirects.";
      case -24:
        return "The download failed due to unsafe redirect.";
      default:
        // Check for common error descriptions
        if (description.toLowerCase().includes('net::err_connection_timed_out')) {
          return "The connection timed out. Please check your internet connection and try again.";
        } else if (description.toLowerCase().includes('net::err_name_not_resolved')) {
          return "The website could not be found. Please check the address and try again.";
        } else if (description.toLowerCase().includes('net::err_connection_refused')) {
          return "The connection was refused by the server. The website may be temporarily unavailable.";
        } else if (description.toLowerCase().includes('net::err_ssl_protocol_error')) {
          return "There was a problem with the website's security certificate. This website may not be secure.";
        } else if (description.toLowerCase().includes('net::err_network_changed')) {
          return "Your network connection changed. Please try reloading the page.";
        } else if (description.toLowerCase().includes('net::err_insecure_response')) {
          return "The website sent an insecure response. This website may not be secure.";
        } else if (description.toLowerCase().includes('net::err_blocked_by_client')) {
          return "The website was blocked by your browser settings.";
        } else if (description.toLowerCase().includes('net::err_blocked_by_administrator')) {
          return "The website was blocked by your network administrator.";
        } else if (description.toLowerCase().includes('net::err_internet_disconnected')) {
          return "You are not connected to the internet. Please connect and try again.";
        } else if (description.toLowerCase().includes('net::err_proxy_connection_failed')) {
          return "Could not connect through the proxy server. Please check your network settings.";
        } else if (description.toLowerCase().includes('net::err_empty_response')) {
          return "The server returned an empty response. Please try again later.";
        } else if (description.toLowerCase().includes('net::err_unknown_url_scheme')) {
          return "The URL scheme is not supported. Please check the address.";
        } else if (description.toLowerCase().includes('net::err_invalid_url')) {
          return "The URL is invalid. Please check the address.";
        } else if (description.toLowerCase().includes('net::err_aborted')) {
          return "The request was aborted. Please try again.";
        } else if (description.toLowerCase().includes('net::err_address_unreachable')) {
          return "The website address is unreachable. Please check your internet connection.";
        } else if (description.toLowerCase().includes('net::err_file_not_found')) {
          return "The page could not be found. It may have been moved or deleted.";
        } else if (description.toLowerCase().includes('net::err_temporarily_throttled')) {
          return "The request was temporarily throttled by the server. Please try again.";
        } else if (description.toLowerCase().includes('net::err_http_response_code_failure')) {
          return "The server returned an error response. The website may be experiencing issues.";
        } else {
  ...
  // Function to check if we should block a deep link to prevent loops
  const shouldBlockDeepLinkLoop = useCallback((url: string): boolean => {
    const now = Date.now();

    // Create a pattern-based hash to catch variations of the same deep link
    // For example: fb://fullscreen_video/3113402345512065?loop=false&wtsid=wt_... should match other wtsid variations
    let urlPattern = url;
    if (url.startsWith('fb://')) {
      // Remove varying parameters like wtsid
      urlPattern = url.replace(/wtsid=[^&]*/g, 'wtsid=VARIATION');
    } else if (url.includes('://')) {
      // For other deep links, remove common tracking parameters
      urlPattern = url.replace(/[?&](wtsid|ref|utm_.*)=[^&]*/g, '');
    }

    const lastAttempt = recentDeepLinkAttempts.current.get(urlPattern) || 0;
    ...
    // Record this attempt with the pattern
    recentDeepLinkAttempts.current.set(urlPattern, now);

    // Clean up old attempts (older than 10 seconds)
    recentDeepLinkAttempts.current.forEach((timestamp, key) => {
      if (now - timestamp > 10000) {
        recentDeepLinkAttempts.current.delete(key);
      }
    });
  ...
  // Clean up blocked URLs periodically to prevent memory leaks
  useEffect(() => {
    const interval = setInterval(() => {
      const now = Date.now();
      // Clean up blocked URLs that are older than 10 seconds
      blockedUrls.current.forEach((timestamp, url) => {
        if (now - timestamp > 10000) {
          blockedUrls.current.delete(url);
        }
      });
  ...
  // Function to inject JavaScript to prevent deep link initiation on web pages
  const preventDeepLinkInitiationScript = `
    (function() {
      // Prevent Facebook from initiating deep links
      if (location.hostname.includes('facebook.com') || location.hostname.includes('fb.com')) {
        // Override window.location assignment to prevent deep link redirects
  ...
  const handleShouldStartLoadWithRequest = useCallback((request: { url: string }) => {
    ...
    if (url.startsWith('fb://')) {
      ...
      // Set flag to prevent duplicate handling in navigation state change
      justBlockedUrl.current = true;

      // Simply block the deep link without navigation - JavaScript should prevent the redirect
      // The page will remain on the current valid page

      // Inject JavaScript to prevent future deep link attempts on the current page
      setTimeout(() => {
        if (webViewRef.current) {
          const preventDeepLinksJS = `
            (function() {
              // Prevent any further deep link attempts
              if (window.location && window.location.href && window.location.href.startsWith('fb://')) {
                console.log('Blocked deep link attempt at window.location level');
                return;
              }

              // Override window.location to prevent deep link redirects
              var originalLocation = window.location;
              Object.defineProperty(window, 'location', {
                configurable: false,
                get: function() {
                ...
                set: function(value) {
                  if (typeof value === 'string' && value.startsWith('fb://')) {
                    console.log('Blocked deep link redirect:', value);
                    return;
                  }
                  originalLocation = value;
                }
              ...
              // Prevent deep link attempts in anchor tags
              document.addEventListener('click', function(e) {
                var target = e.target;
                while (target && target.tagName !== 'A') {
                  target = target.parentElement;
                ...
                if (target && target.href && target.href.startsWith('fb://')) {
                  e.preventDefault();
                  e.stopPropagation();
                  console.log('Blocked deep link in anchor click:', target.href);
                  return false;
                }
              }, true);
            })();
            true;
          `;
          webViewRef.current.injectJavaScript(preventDeepLinksJS);
        }
      }, 100);
    ...
    if (specialSchemes.includes(scheme)) {
      ...
      if (webUrl) {
        ...
        if (!result.blocked) {
          // Navigate to the web version in current tab
          if (activeTabId) {
            updateTab(activeTabId, { url: result.url, sourceUrl: result.url });
          }
          setForceNavCounter(c => c + 1); // Force WebView to navigate
        ...
        // If no web fallback available, try to open in external app
            ...
            if (supported) {
              ...
              setTimeout(() => {
                if (webViewRef.current && webViewRef.current.canGoBack()) {
                  ...
                  // If can't go back, reload the current page to clear the error
                  webViewRef.current?.reload();
                }
              }, 100);
            }
        ...
        return false; // Prevent the original deep link from loading in WebView
      }
    }
    ...
    // Treat WebView requests as resources/sub-requests to avoid spamming alerts for ads/tracking
    // User-initiated navigation (typing URL, clicking link) is handled separately
    const result = processUrl(request.url, 'resource');
    if (result.blocked) {
      // Handle redirect if specified (e.g., redirect to safesearchengine.com)
      if (result.redirect) {
        ...
        // Only show alert if this is a user-initiated navigation
        Alert.alert('Content Blocked', result.reason || 'This content is blocked');
      }
      return false;
    }
  ...
  const clearError = useCallback(() => {
    ...
    setErrorMessage('');
    setErrorDetails({ domain: '', code: '', description: '' });
  }, [setHasError, setErrorMessage, setErrorDetails]);
  ...
  const injectedJS = isGoogleAuth ? scrollTrackingJS : (
    (isReddit
      ...
      : isChatGPT
        ...
        : scrollTrackingJS) + notificationInterceptionJS + getPermissionBlockingScript(currentHostname) + mediaFilterJS + facebookDeepLinkPreventionJS
  );
            ...
            <View style={[styles.linkContextMenuBox, { backgroundColor: theme.surface, borderColor: theme.border }]}>
              ...
              <Pressable
                onPress={() => {
                  ...
                  const result = processUrl(linkContextMenu.url, 'navigation');
                  if (!result.blocked) {
                    createTab(result.url);
                  } else if (result.showAlert) {
                    Alert.alert('Content Blocked', result.reason || 'This content is blocked');
            ...
            <Animated.View key={tab.id} style={[styles.webViewContainer, webViewContainerAnimatedStyle, tab.id === activeTabId ? {} : { display: 'none' }]}>
              <WebView
                ...
                (function() {
                  ...
                  // Enhanced Facebook deep link prevention - intercept ALL navigation attempts to fb://
                  if (location.hostname.includes('facebook.com') || location.hostname.includes('fb.com') || location.hostname.includes('mbasic.facebook.com')) {
                    // Intercept location.assign and location.replace
                    const originalAssign = window.location.assign;
                    const originalReplace = window.location.replace;

                    window.location.assign = function(url) {
                      if (typeof url === 'string' && url.startsWith('fb://')) {
                        console.log('Blocked location.assign to fb://');
                        return;
                      }
                    ...
                    window.location.replace = function(url) {
                      if (typeof url === 'string' && url.startsWith('fb://')) {
                        console.log('Blocked location.replace to fb://');
                        return;
                      }
                    ...
                    Object.defineProperty(window, 'location', {
                      ...
                      set: function(value) {
                        if (typeof value === 'string' && value.startsWith('fb://')) {
                          console.log('Blocked location setter to fb://');
                          return;
                        }
                    ...
                    // Block window.open to fb:// URLs
                    const originalOpen = window.open;
                    window.open = function(url, ...args) {
                      if (typeof url === 'string' && url.startsWith('fb://')) {
                        console.log('Blocked window.open to fb://');
                        return null;
                      }
                      return originalOpen.call(this, url, ...args);
                    };

                    // Intercept clicks on links that lead to fb:// URLs
                    document.addEventListener('click', function(e) {
                      let target = e.target;
                      while (target && target.tagName !== 'A') {
                        target = target.parentElement;
                      ...
                      if (target && target.href && target.href.startsWith('fb://')) {
                        e.preventDefault();
                        e.stopPropagation();
                        console.log('Blocked click on fb:// link');
                        return false;
                      }
                    }, true);
                  }
              ...
              ` + (skipHeavyScripts ? DEBUG_CONSOLE_PROXY_JS + scrollTrackingJS : injectedJS) + (!skipHeavyScripts && tabIsYouTube ? youtubeFixJS : '') + (!skipHeavyScripts && tabIsReddit ? REDDIT_NSFW_FILTER_JS : '') + (!skipHeavyScripts && tabIsGoogle ? GOOGLE_SAFESEARCH_SUPPRESSION_JS + GOOGLE_SECTIONS_BLOCK_JS : '')}
                ...
                onReceivedError={(syntheticEvent: any) => {
                  ...
                  if (nativeEvent.code === -10 && nativeEvent.description.includes('net::ERR_UNKNOWN_URL_SCHEME') && nativeEvent.url.startsWith('fb://')) {
                    ...
                    // Simply block the deep link without conversion - stay on current page
                    // Don't set error state for Facebook deep link issues
                    // Reset loading state to ensure page continues to work
                    setIsLoading(false);
                    isLoadingRef.current = false;
                    loadingProgress.value = withTiming(1, { duration: 200 }, () => {
                      loadingProgress.value = 0;
                    ...
                    // Set flag to prevent duplicate handling elsewhere
                    justBlockedUrl.current = true;
                  ...
                  // Set error state with friendly message for non-Facebook errors
                  const friendlyMessage = getFriendlyErrorMessage(nativeEvent.code, nativeEvent.description, nativeEvent.url);
                  setErrorMessage(friendlyMessage);
                  setErrorDetails({
                    domain: nativeEvent.url || 'unknown',
                    code: nativeEvent.code.toString(),
                ...
                onReceivedHttpError={(syntheticEvent: any) => {
                  ...
                  const description = nativeEvent.description || `HTTP Error ${errorCode}`;

                  // For HTTP errors, we'll use specific handling since they're not WebView-specific error codes
                  let friendlyMessage = `HTTP Error ${errorCode}`;
                  if (errorCode === 404) {
                    friendlyMessage = "The page could not be found. It may have been moved or deleted.";
                  } else if (errorCode === 403) {
                    friendlyMessage = "Access to this page is forbidden.";
                  } else if (errorCode === 401) {
                    friendlyMessage = "Unauthorized access. You may need to log in.";
                  } else if (errorCode >= 500 && errorCode < 600) {
                    friendlyMessage = "The server is temporarily unavailable. Please try again later.";
                  } else if (errorCode >= 400 && errorCode < 500) {
                    friendlyMessage = "The request could not be processed. The page may have been removed or you may not have access.";
                  } else {
                    // For other cases, fall back to the general error handling
                    friendlyMessage = getFriendlyErrorMessage(-errorCode, description, nativeEvent.url);
                  }
                  setErrorMessage(friendlyMessage);
                  setErrorDetails({
                    domain: nativeEvent.url || 'unknown',
                    code: nativeEvent.statusCode.toString(),
                    description: nativeEvent.description
                  });
                  setHasError(true);
                }}
              ...
              />
              {/* Error Overlay - Shown when there's a loading error */}
              {hasError && (
                <Animated.View
                  style={[
                    styles.webViewContainer,
                    webViewContainerAnimatedStyle,
                    {
                      position: 'absolute',
                      top: 0,
                      left: 0,
                ...
                >
                  <View style={styles.errorContainer}>
                    <Feather name="wifi-off" size={80} color={theme.textSecondary} style={styles.errorIcon} />
                    <ThemedText style={styles.errorTitle}>Unable to Load Page</ThemedText>
                    ...
                    >
                      <ThemedText style={styles.errorRetryButtonText}>Try Again</ThemedText>
                    </Pressable>
                  </View>
                </Animated.View>
              )}
